# -*- coding: utf-8 -*-
"""
Analisador de Apostas em Escanteios - v6.1 (Time-Dependent Poisson Model)
Interface Web (Streamlit)
Modelo de Poisson com Média Ponderada + Taxa Variável (Time-Dependent)
...
st.set_page_config(
    page_title="Analisador de Escanteios v6.1",
    page_icon="⚽",
    layout="wide",
...
def calcular_ev_e_prob(minutos_jogados, escanteios_atuais, linha_over, odd_over, odd_under, media_escanteios_por_jogo, fator_inicio, fator_fim_1t, fator_inicio_2t, fator_fim_jogo):
    """
    Calcula as probabilidades e o Valor Esperado (EV) para a aposta em escanteios.
    Usa o Modelo de Taxa Variável (Time-Dependent Poisson Model).
    """
...
st.title("⚽ Analisador de Apostas em Escanteios v6.1")
st.markdown("**Modelo de Poisson com Taxa Variável (Time-Dependent)**")
...
with st.sidebar:
    ...
    fator_fim_jogo = st.slider(
        ...
        value=1.20,
        step=0.05,
        help="Fator de ponderação para o fim do jogo (padrão: 1.30, reduzido para 1.20 por conservadorismo)"
    )
...
# --- Informação sobre Fatores Temporais ---

with st.expander("📅 Fatores de Ponderação Temporal (Time-Dependent)"):
    st.markdown("""
    O modelo v6.1 ajusta a taxa de escanteios ao longo do tempo, reconhecendo que a taxa não é constante. Os fatores abaixo são os que estão sendo usados no cálculo:
    
    | Zona | Minutos | Fator |
    | :--- | :--- | :--- |
    | **Início** | 0 - 35 | {:.2f} |
    | **Fim 1º Tempo** | 36 - 45 | {:.2f} |
    | **Início 2º Tempo** | 46 - 80 | {:.2f} |
    | **Fim/Acréscimos** | 81 - 95 | {:.2f} |
    """.format(fator_inicio, fator_fim_1t, fator_inicio_2t, fator_fim_jogo))

st.markdown("---")

# --- Botão de Cálculo ---

if st.button("🔍 Calcular Análise (v6.1)", use_container_width=True, type="primary"):
    ...
    if minutos_jogados < 0 or minutos_jogados > 95:
        ...
        st.markdown("---")
        st.subheader("📊 Resultados da Análise (v6.1)")
        ...
        with st.expander("📋 Detalhes Técnicos (v6.1)"):
            ...
            **Projeções (Time-Dependent):**
            ...
            - (Calculado com fatores de ponderação temporal)
            
            **Modelo Estatístico:**
            - Distribuição: Poisson (Time-Dependent)
...
<p><strong>Analisador de Apostas em Escanteios v6.1</strong> | Time-Dependent Poisson Model</p>
