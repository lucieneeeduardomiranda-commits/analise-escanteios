...
def get_temporal_factor_ht(minuto, f_ini, f_meio, f_final, f_extras):
    ...
    else: return f_extras # Minutos 46, 47, 48

def calcular_lambda_restante_ht(minutos_jogados, media_ht, f1, f2, f3, f4):
    tempo_base = 45
    tempo_limite = 48 # Foco no HT estendido
...
st.info("Modelo ajustado para calcular valor real no 1º Tempo considerando acréscimos.")

c1, c2 = st.columns(2)
mcf = c1.number_input("Média Casa no HT", 0.0, 10.0, 2.5)
mvf = c2.number_input("Média Visitante no HT", 0.0, 10.0, 2.0)

media_ponderada = mcf + mvf
...
if st.button("CALCULAR PROBABILIDADES HT"):
    ...
    # Detalhes Técnicos
    with st.expander("🔍 Parâmetros da Projeção"):
        st.write(f"**Lambda Restante:** {lambda_r:.4f}")
        st.write(f"**Target Over:** {target_over} escanteios extras")
        st.write(f"**K (Dispersão):** {k_val}")
        st.write(f"**Linha de entrada:** {linha}")